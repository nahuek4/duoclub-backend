...
function applyHistoricalPriceConsensus(candidates = [], periodKey = "") {
  ...
  for (const candidate of candidates) {
    ...
    // Solo usamos como evidencia usuarios que efectivamente tienen pagos
    // compatibles suficientes para cubrir al menos el total viejo del ciclo.
    // Así un pago parcial no deforma el precio histórico del grupo.
    if (!(received > 0) || !(oldTotal > 0) || received < oldTotal) continue;
...
async function main() {
  ...
  try {
    ...
    console.log("\n" + "=".repeat(110));
    console.log(`PREVIEW V4 REPARACIÓN SUSCRIPCIONES / TURNOS FIJOS · ${periodKey}`);
    ...
    const outPath = path.join(
      outDir,
      `repair-preview-v4-${periodKey}-${stamp}.json`
    );
