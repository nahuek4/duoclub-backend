...
//
// USO:
//   node scripts/previewSubscriptionRepairV4.js --period=2026-09
//   node scripts/repairSubscriptionsFromPreviewV4.js --period=2026-09
//   node scripts/repairSubscriptionsFromPreviewV4.js --period=2026-09 --apply
//
// Opcional:
...
function newestPreviewReport(periodKey) {
  ...
  if (!fs.existsSync(dir)) {
    throw new Error(
      `No existe ${dir}. Corré primero previewSubscriptionRepairV4.js.`
    );
  }

  const prefix = `repair-preview-v4-${periodKey}-`;
  ...
  if (!files.length) {
    throw new Error(
      `No encontré preview V4 para ${periodKey}. Corré primero: node scripts/previewSubscriptionRepairV4.js --period=${periodKey}`
    );
  }
  ...
  if (ageMs > maxAgeMs) {
    throw new Error(
      `El preview tiene ${Math.round(
        ageMs / 60000
      )} minutos. Volvé a correr V4 antes de aplicar (máximo ${maxAgeMinutes} min).`
    );
  }
...
async function main() {
  ...
  try {
    ...
    if (!apply) {
      ...
      console.log(
        `node scripts/repairSubscriptionsFromPreviewV4.js --period=${periodKey} --apply`
      );
      return;
    }
