...
// Reparación controlada de los 4 usuarios con pago parcial de septiembre que, con la nueva regla, deben permanecer activos.
//
// SEGURIDAD:
...
function readReport(reportPath, periodKey, maxAgeMinutes) {
  ...
  for (const row of rows) {
    ...
    if (!(row?.claims?.length > 0)) {
      throw new Error(
        `No encontré appointments futuros recuperables para ${email}`
      );
    }
  }
...
async function main() {
  ...
  try {
    ...
    if (!apply) {
      ...
      console.log(
        `node scripts/repairSeptemberPartialPaymentsKeepActive.js --period=${periodKey} --apply`
      );
      return;
    }
