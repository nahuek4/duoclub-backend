// scripts/repairThirteenPaidCreditsV1.js
//
...
//
// USO:
//   node scripts/repairThirteenPaidCreditsV1.js --period=2026-09
//   node scripts/repairThirteenPaidCreditsV1.js --period=2026-09 --apply
//
// Opcional:
...
async function buildRow(target, periodKey, bounds, today) {
  ...
  const fixedOccurrenceRows = relevantAppointments.filter(
    (ap) =>
      !!ap.fixedScheduleId &&
      candidateScheduleIds.includes(String(ap.fixedScheduleId)) &&
      clean(ap.date).slice(0, 10) >= bounds.startYmd &&
      clean(ap.date).slice(0, 10) <= bounds.endYmd &&
      !(
        clean(ap.status).toLowerCase() === "cancelled" &&
        ap.refundApplied === true
      )
  ...
  const fixedOccurrencesCount = fixedOccurrenceRows.length;
...
async function applyRow(row, periodKey) {
  ...
  try {
    await session.withTransaction(async () => {
      ...
      const fixedOccurrenceRows =
        relevantAppointments.filter(
          (ap) =>
            !!ap.fixedScheduleId &&
            candidateScheduleIds.includes(
              String(ap.fixedScheduleId)
            ) &&
            clean(ap.date).slice(0, 10) >=
              `${periodKey}-01` &&
            clean(ap.date).slice(0, 10) <=
              periodBounds(periodKey).endYmd &&
            !(
              clean(ap.status).toLowerCase() ===
                "cancelled" &&
              ap.refundApplied === true
...
function printRow(row) {
  ...
  console.log(
    `  Fijos del período=${row.fixedOccurrencesCount || 0} | ` +
...
async function main() {
  ...
  try {
    ...
    if (!apply) {
      ...
      if (readyRows.length) {
        console.log(
          `Para aplicar SOLO los READY: node scripts/repairThirteenPaidCreditsV1.js --period=${periodKey} --apply`
        );
      }
