...
function scorePaidOrderMatch({ order, item, subscription, cycle }) {
  ...
  const creditsExact = expectedCredits > 0 && asInt(item?.credits) === expectedCredits;
  const methodExact = !!expectedMethod && orderMethod === expectedMethod;
  const planExact =
    !!expectedPlanId &&
  ...
  const periodExact =
    !!clean(item?.periodKey) && clean(item?.periodKey) === clean(cycle?.periodKey);

  let score = 0;
  if (linkedCycleExact) score += 10;
  if (linkedSubscriptionExact) score += 5;
  if (creditsExact) score += 3;
  if (methodExact) score += 2;
  if (planExact) score += 3;
  if (amountExact) score += 3;
  if (periodExact) score += 3;
...
async function buildServiceCandidate({
  ...
  const unpaidCycle = ["pending", "overdue"].includes(clean(cycle?.billing?.status).toLowerCase());
  ...
  matches.sort((a, b) => {
    if (a.match.strong !== b.match.strong) return a.match.strong ? -1 : 1;
    return b.match.score - a.match.score;
  });

  const best = matches[0] || null;
  const strongPaymentMatch = Boolean(best?.match?.strong);
  ...
  for (const schedule of schedules) {
    const originalEnd = clean(schedule?.endDate);
    const horizonEnd = addDaysYmd(now.ymd, horizonDays);
    const checkEnd = originalEnd && originalEnd < horizonEnd ? originalEnd : horizonEnd;
    const checkStart =
      clean(schedule?.startDate) > now.ymd ? clean(schedule.startDate) : now.ymd;

    const scheduleClaims = [];

    for (const item of Array.isArray(schedule?.items) ? schedule.items : []) {
      const dates = enumerateDates(checkStart, checkEnd, Number(item?.weekday));

      for (const date of dates) {
        const time = clean(item?.time).slice(0, 5);
        if (!time || isPastSlot(date, time, now)) continue;
        ...
        scheduleClaims.push({
          claimId: `${String(user._id)}:${sk}:${String(schedule._id)}:${date}:${time}`,
          ...
          date,
          time,
          targetAppointmentId: idOf(targetAppointment),
          action:
            targetAppointment ? "RESTORE_CANCELLED_APPOINTMENT" : "CREATE_APPOINTMENT_IF_NEEDED",
          strongPaymentMatch,
        });
      }
    }
    ...
    scheduleRows.push({
      ...
      claimsCount: scheduleClaims.length,
    });

    claims.push(...scheduleClaims);
  }
  ...
  const creditAudit = {
    ...
    note:
      cycle?.creditGrant?.invalidatedAt
        ? "El lote fue invalidado por la baja. El APPLY deberá reconstruir el saldo sin duplicar sesiones consumidas."
  ...
  return {
    ...
    paymentMatch: best
      ? {
          strong: Boolean(best.match.strong),
          score: best.match.score,
          orderId: idOf(best.order),
          orderPaidAt: best.order?.paidAt || best.order?.createdAt || null,
          orderTotal: money(best.order?.totalFinal ?? best.order?.total ?? best.order?.price),
...
function compactCandidate(candidate, evaluatedClaims) {
  ...
  if (candidate.repairEligibleBase) {
    if (!claims.length) {
      decision = "REVIEW_NO_FUTURE_OCCURRENCES";
...
async function main() {
  ...
  try {
    ...
    console.log("\n" + "=".repeat(110));
    console.log(`PREVIEW REPARACIÓN SUSCRIPCIONES / TURNOS FIJOS · ${periodKey}`);
    ...
    for (const row of rows) {
      ...
      console.log(
        `  Pago compatible: ${row.strongPaymentMatch ? "SÍ" : "NO"}${
          row.paymentMatch ? ` | orden=${row.paymentMatch.orderId} | score=${row.paymentMatch.score}` : ""
        }`
      );
      console.log(
    ...
    const outPath = path.join(
      outDir,
      `repair-preview-${periodKey}-${stamp}.json`
    );
