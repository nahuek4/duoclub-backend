...
async function main() {
  ...
  const users = await User.find({})
    .select(
      "name lastName email phone credits creditLots fixedScheduleDebt firstEvaluationCompleted role approvalStatus suspended"
    )
    .lean();
  ...
  for (const row of rows) {
    for (const sk of SERVICE_KEYS) {
      totals.activeCreditsByService[sk] += n(row.activeCreditsByService[sk]);
    }
    for (const sk of ["EP", "RA", "RF", "KD", "SYN"]) {
      totals.fixedScheduleDebt[sk] += n(row.fixedScheduleDebt[sk]);
    }
  }
  ...
  const output = {
    ...
    readOnly: true,
    note:
      "Snapshot previo/post cambio. Este script no ejecuta save(), update(), delete() ni bulkWrite().",
    totals,
    users: rows,
  };
  ...
  const outPath = path.join(
    outDir,
    `duo-integrity-snapshot-${ymdhms()}.json`
  );
