...
async function buildServiceCandidate({
  ...
  return {
    ...
    cycle: {
      id: idOf(cycle),
      periodKey: clean(cycle?.periodKey),
      planMonthlySessions: asInt(cycle?.planSnapshot?.monthlySessions),
      planPayMethod: clean(cycle?.planSnapshot?.payMethod).toUpperCase(),
      planPricingPlanId: idOf(cycle?.planSnapshot?.pricingPlan),
      planBasePrice: money(cycle?.planSnapshot?.basePrice),
      billingStatus: clean(cycle?.billing?.status),
      billingTotal: money(cycle?.billing?.total),
      billingOrder: idOf(cycle?.billing?.order),
      coverageApplied: Boolean(cycle?.planSnapshot?.coverageApplied),
    ...
    creditAudit,
    claims,
    historicalPlanIdentity: {
      cycleSessions:
        asInt(cycle?.planSnapshot?.monthlySessions) ||
        asInt(cycle?.creditGrant?.grantedSessions),
      creditGrantSessions: asInt(cycle?.creditGrant?.grantedSessions),
      currentSubscriptionSessions: asInt(subscription?.monthlySessions),
      cyclePayMethod: clean(cycle?.planSnapshot?.payMethod).toUpperCase(),
      currentSubscriptionPayMethod: clean(subscription?.payMethod).toUpperCase(),
      sessionsChangedSinceCycle:
        asInt(cycle?.planSnapshot?.monthlySessions) > 0 &&
        asInt(subscription?.monthlySessions) > 0 &&
        asInt(cycle?.planSnapshot?.monthlySessions) !==
...
function historicalPriceConsensusKey(candidate) {
  const historicalSessions =
    asInt(candidate?.cycle?.planMonthlySessions) ||
    asInt(candidate?.creditAudit?.grantedSessions) ||
    asInt(candidate?.subscription?.monthlySessions);

  const historicalPayMethod =
    clean(candidate?.cycle?.planPayMethod).toUpperCase() ||
    clean(candidate?.subscription?.payMethod).toUpperCase();

  return [
    clean(candidate?.serviceKey).toUpperCase(),
    historicalSessions,
    historicalPayMethod,
    candidate?.cycle?.coverageApplied ? "COVERAGE" : "REGULAR",
  ].join("|");
}

function applyHistoricalPriceConsensus(candidates = [], periodKey = "") {
  const evidenceByGroup = new Map();

  for (const candidate of candidates) {
    const groupKey = historicalPriceConsensusKey(candidate);
    const ledger = candidate?.paymentLedgerPreview || {};
    const received = money(ledger.amountReceivedForPlan);
    const oldTotal = money(ledger.cycleBillingTotal);
    ...
    const compatiblePayments = Array.isArray(ledger?.compatiblePayments)
      ? ledger.compatiblePayments
      : [];

    const paymentMatchesHistoricalPlan = compatiblePayments.some(
      (payment) =>
        asInt(payment?.credits) === historicalSessions &&
        clean(payment?.payMethod).toUpperCase() === historicalPayMethod
    );

    // Solo usamos como evidencia pagos compatibles con EL SNAPSHOT DEL CICLO:
    // mismas sesiones históricas + mismo medio de pago.
    // Esto evita agrupar por el plan actual de la suscripción si cambió.
    if (
      !(received > 0) ||
      !(oldTotal > 0) ||
      received < oldTotal ||
      ...
      !historicalPayMethod ||
      !paymentMatchesHistoricalPlan
    ) {
      continue;
    }

    if (!evidenceByGroup.has(groupKey)) {
      evidenceByGroup.set(groupKey, []);
    }

    evidenceByGroup.get(groupKey).push({
      amount: received,
      email: clean(candidate?.user?.email),
      userId: clean(candidate?.user?.id),
    });
  }

  const consensusByGroup = new Map();

  for (const [groupKey, rows] of evidenceByGroup.entries()) {
    ...
    const [amount, count] = ranking[0] || [0, 0];
    const share = rows.length ? count / rows.length : 0;

    // Necesitamos al menos dos casos coincidentes y mayoría clara.
    // Evita "inventar" precio histórico por una sola orden atípica.
    if (amount > 0 && count >= 2 && share >= 0.6) {
      consensusByGroup.set(groupKey, {
        amount,
        count,
  ...
  for (const candidate of candidates) {
    const groupKey = historicalPriceConsensusKey(candidate);
    const consensus = consensusByGroup.get(groupKey);

    if (!consensus) continue;

    const ledger = candidate.paymentLedgerPreview;
    const oldExpected = money(ledger.expectedPlanAmount);

    // Para la reparación de septiembre priorizamos el consenso histórico
    // del grupo si el ciclo fue generado con otro precio.
    ledger.expectedPlanAmount = consensus.amount;
    ledger.authoritativeAmountSource =
      `HISTORICAL_COHORT_CONSENSUS:${groupKey}`;
    ledger.consensusEvidence = consensus;

    const received = money(ledger.amountReceivedForPlan);
    ledger.amountAppliedToPlan = Math.min(consensus.amount, received);
    ledger.balanceDuePreview = Math.max(
      0,
      consensus.amount - ledger.amountAppliedToPlan
    );
    ledger.excessPreview = Math.max(
      0,
      received - consensus.amount
    );
    ledger.fullyPaid =
      consensus.amount > 0 && received >= consensus.amount;
    ledger.partial =
      consensus.amount > 0 &&
      received > 0 &&
      received < consensus.amount;
    ledger.cycleTotalNeedsCorrection =
      money(ledger.cycleBillingTotal) !== consensus.amount;

    candidate.paymentMatch.expectedPlanAmount = consensus.amount;
    candidate.paymentMatch.authoritativeAmountSource =
      ledger.authoritativeAmountSource;
    candidate.paymentMatch.amountAppliedToPlan =
      ledger.amountAppliedToPlan;
    candidate.paymentMatch.balanceDuePreview =
      ledger.balanceDuePreview;
    candidate.paymentMatch.excessPreview =
      ledger.excessPreview;
    candidate.paymentMatch.fullyPaidByAccumulation =
      ledger.fullyPaid;
    candidate.paymentMatch.partiallyPaidByAccumulation =
      ledger.partial;

    candidate.strongPaymentMatch = ledger.fullyPaid;

    if (ledger.fullyPaid) {
      candidate.paymentMatchReason =
        "FULLY_PAID_BY_HISTORICAL_PRICE_CONSENSUS";
    } else if (ledger.partial) {
      candidate.paymentMatchReason =
        "PARTIAL_PAYMENT_BY_HISTORICAL_PRICE_CONSENSUS";
    } else {
      candidate.paymentMatchReason =
        "NO_PAYMENT_BY_HISTORICAL_PRICE_CONSENSUS";
    }

    candidate.paymentMatch.reason = candidate.paymentMatchReason;

    candidate.repairEligibleBase =
      candidate.unpaidCycle &&
      candidate.blockedSubscription &&
      candidate.strongPaymentMatch &&
      candidate.releasedFixedSchedules.length > 0;

    for (const claim of candidate.claims) {
      claim.strongPaymentMatch = candidate.strongPaymentMatch;
    }

    if (
      oldExpected !== consensus.amount ||
      ledger.cycleTotalNeedsCorrection
    ) {
      corrected += 1;
    }
  ...
  return {
    groupsWithEvidence: evidenceByGroup.size,
    consensusGroups: consensusByGroup.size,
    correctedCandidates: corrected,
    groups: Object.fromEntries(consensusByGroup.entries()),
  };
}
...
async function main() {
  ...
  try {
    ...
    const historicalPriceConsensus = applyHistoricalPriceConsensus(
      candidates,
      periodKey
    );
    ...
    console.log("\n" + "=".repeat(110));
    console.log(`PREVIEW V5 REPARACIÓN SUSCRIPCIONES / TURNOS FIJOS · ${periodKey}`);
    ...
    for (const row of rows) {
      ...
      if (row.historicalPlanIdentity?.sessionsChangedSinceCycle) {
        console.log(
          `  ! sesiones actuales=${row.historicalPlanIdentity.currentSubscriptionSessions} pero ciclo ${periodKey}=${row.historicalPlanIdentity.cycleSessions}; V5 usa SIEMPRE el ciclo histórico`
        );
      }
      console.log(
    ...
    const summary = {
      ...
      subscriptionsWhoseCurrentSessionsDifferFromCycle: rows.filter(
        (row) => row.historicalPlanIdentity?.sessionsChangedSinceCycle
      ).length,
    ...
    const outPath = path.join(
      outDir,
      `repair-preview-v5-${periodKey}-${stamp}.json`
    );
